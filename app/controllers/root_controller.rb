class RootController < ApplicationController
  def index
    ...
    @stories = process_filter_querystring(Story.is_not_deleted.is_published.in_published_theme).with_translations(I18n.locale).paginate(:page => p, :per_page => per_page)
    @theme = Theme.for_homepage

    @highlight = Highlight.picked
    # it may require is_not_deleted
  ...
  def author
    ...
    if @author.present?
      ...
      @stories = process_filter_querystring(Story.is_not_deleted.by_authors(@author.id).in_published_theme).with_translations(I18n.locale).paginate(:page => params[:page], :per_page => per_page)
  ...
  def embed
    ...
    if sl.present? && I18n.locale.to_s != sl && I18n.available_locales.index(sl.to_sym)
      ...
      @story = Story.with_translations(I18n.locale).is_not_deleted.is_published.in_published_theme.find_by_permalink(params[:story_id])
  ...
  def theme
    ...
    if @theme.present?
      @stories = process_filter_querystring(Story.is_not_deleted.is_published.in_published_theme.by_theme(@theme.id)).with_translations(I18n.locale).paginate(:page => params[:page], :per_page => per_page)
